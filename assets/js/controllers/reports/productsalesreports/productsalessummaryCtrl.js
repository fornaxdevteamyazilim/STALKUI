...
function productsalessummaryCtrl($scope, $filter, $modal, $log,  Restangular, $translate, SweetAlert, toaster, $window, $rootScope, $compile, $timeout, $location, userService, ngnotifyService, $element, NG_SETTING) {
    ...
    if (!$scope.fromDate) {
        $scope.fromDate = moment().add(-1, 'days').format('YYYY-MM-DD ');
    }
    if (!$scope.toDate) {
        $scope.toDate = $filter('date')(ngnotifyService.ServerTime(), 'yyyy-MM-dd ');
    }
    ...
    $scope.pivotGridOptions = {
        ...
        dataSource: {
            ...
            store: DevExpress.data.AspNet.createStore({
                key: "id",
                loadUrl: NG_SETTING.apiServiceBaseUri + "/api/reports/productsalessummary",
            }),
            filter: getFilter(),
            fields: [
